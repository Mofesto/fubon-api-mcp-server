...
### `analysis_service_live_readonly.py`

此腳本使用 `.env` 的真實富邦登入資料執行 Phase 3 唯讀驗收：

- 以正式歷史行情計算指定標的的技術／成交量情緒。
- 以正式帳戶庫存計算 VaR；空倉時預期回報 `insufficient_data`。
- SDK 外層會攔截下單、改單、刪單等交易寫入方法。
- 輸出只顯示工具狀態、資料來源及時間，不列印帳號或憑證。

```powershell
python examples/analysis_service_live_readonly.py
```
