...
class TestReportsServices:
    ...
    def test_get_order_results_success(self, mock_sdk, mock_accounts):
        ...
        mock_result.data = [{"order_no": "12345", "symbol": "2330", "quantity": 1000, "price": 500.0, "status": "Filled"}]
        mock_sdk.stock.get_order_results.return_value = mock_result

        result = (
            get_order_results.fn({"account": "12345678"})
            if hasattr(get_order_results, "fn")
            else get_order_results({"account": "12345678"})
        )
    ...
    def test_get_order_results_account_not_found(self, mock_accounts):
        ...
        result = (
            get_order_results.fn({"account": "99999999"})
            if hasattr(get_order_results, "fn")
            else get_order_results({"account": "99999999"})
        )
    ...
    def test_get_order_changed_reports_success(self, mock_sdk):
        ...
        result = (
            get_order_changed_reports.fn({"limit": 5})
            if hasattr(get_order_changed_reports, "fn")
            else get_order_changed_reports({"limit": 5})
        )
    ...
    def test_get_filled_reports_success(self, mock_sdk):
        ...
        result = (
            get_filled_reports.fn({"limit": 20}) if hasattr(get_filled_reports, "fn") else get_filled_reports({"limit": 20})
        )
